...
public class EzQueueUtils {
	 ...
	 */
	public static Map < Integer, String > getPlayersInQueue(Player p) {
	    String queue = getQueue(p);
		QueueInfo info = QueueInfo.getQueueInfo(queue);
	    if (info == null)
	        return null;
	    
	    Map<Integer, String> swapped = info.getPlayersInQueue().entrySet().stream().collect(Collectors.toMap(Map.Entry::getValue, Map.Entry::getKey));
	 ...
	 */
	public static int getQueueSize(Player p) {
	    String queue = getQueue(p);
		QueueInfo info = QueueInfo.getQueueInfo(queue);
